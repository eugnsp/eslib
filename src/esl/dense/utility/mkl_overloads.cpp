...
#define ESL_IMPL_MKL_BLAS_AXPY_R(fn, T)                                                            \
	...
	{                                                                                              \
		::fn(static_cast<MKL_INT>(n),                                                              \
			 a,                                                                                    \
			 data_x,                                                                               \
			 static_cast<MKL_INT>(inc_x),                                                          \
			 data_y,                                                                               \
...
#define ESL_IMPL_MKL_BLAS_DOT(fn, T)                                                               \
	T mkl_dot(const std::size_t n,                                                                 \
			  const T* const data_x,                                                               \
			  const std::size_t inc_x,                                                             \
			  const T* const data_y,                                                               \
			  const std::size_t inc_y)                                                             \
	{                                                                                              \
		return ::fn(static_cast<MKL_INT>(n),                                                       \
					data_x,                                                                        \
					static_cast<MKL_INT>(inc_x),                                                   \
					data_y,                                                                        \
					static_cast<MKL_INT>(inc_y));                                                  \
...
#define ESL_IMPL_MKL_GEMM_R(fn, T)                                                                 \
	void mkl_gemm(const CBLAS_LAYOUT layout,                                                       \
				  ...
				  const CBLAS_TRANSPOSE transp_b,                                                  \
				  const std::size_t m,                                                             \
				  const std::size_t n,                                                             \
				  const std::size_t k,                                                             \
				  const T alpha,                                                                   \
...
#define ESL_IMPL_MKL_OMATCOPY(fn, T)                                                               \
	void mkl_omatcopy(const char layout,                                                           \
					  ...
					  const T alpha,                                                               \
					  const T* const data_a,                                                       \
					  const std::size_t l_dim_a,                                                   \
					  T* const data_b,                                                             \
					  const std::size_t l_dim_b)                                                   \
	{                                                                                              \
		Mkl_type<T> alpha_m;                                                                       \
		...
		::fn(layout,                                                                               \
			 ...
			 cols,                                                                                 \
			 alpha_m,                                                                              \
			 reinterpret_cast<const Mkl_type<T>*>(data_a),                                         \
			 l_dim_a,                                                                              \
			 reinterpret_cast<Mkl_type<T>*>(data_b),                                               \
			 l_dim_b);                                                                             \
	}
...
#define ESL_IMPL_MKL_LAPACK_SYTRD(fn, T)                                                           \
	void mkl_lapack_sytrd(const int layout,                                                        \
						  ...
						  T* const data_tau)                                                       \
	{                                                                                              \
		const auto info = ::fn(layout,                                                             \
							   up_lo,                                                              \
							   static_cast<lapack_int>(n),                                         \
							   data_a,                                                             \
							   static_cast<lapack_int>(lead_dim_a),                                \
							   data_d,                                                             \
							   data_e,                                                             \
							   data_tau);                                                          \
		if (info != 0)                                                                             \
