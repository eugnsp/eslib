...
#define ESL_IMPL_MKL_SPARSE_CREATE_CSR(fn, T)                                                      \
	::sparse_status_t mkl_sparse_create_csr(::sparse_matrix_t& handle, MKL_UINT n_rows,            \
		MKL_UINT n_cols, MKL_UINT* rows_start, MKL_UINT* rows_end, MKL_UINT* col_index, T* values) \
	{                                                                                              \
		return ::fn(&handle, ::sparse_index_base_t::SPARSE_INDEX_BASE_ZERO,                        \
			static_cast<MKL_INT>(n_rows), static_cast<MKL_INT>(n_cols),                            \
...
#define ESL_IMPL_MKL_SPARSE_SET_VALUE(fn, T)                                                       \
	::sparse_status_t mkl_sparse_set_value(                                                        \
		::sparse_matrix_t handle, MKL_UINT row, MKL_UINT col, T value)                             \
	{                                                                                              \
		Mkl_type<T> value_m;                                                                       \
...
#define ESL_IMPL_MKL_SPARSE_EXPORT_CSR(fn, T)                                                      \
	::sparse_status_t mkl_sparse_export_csr(::sparse_matrix_t handle,                              \
		::sparse_index_base_t& indexing, MKL_UINT& n_rows, MKL_UINT& n_cols,                       \
		MKL_UINT*& rows_start, MKL_UINT*& rows_end, MKL_UINT*& col_index, T*& values)              \
	{                                                                                              \
		return ::fn(handle, &indexing, reinterpret_cast<MKL_INT*>(&n_rows),                        \
			reinterpret_cast<MKL_INT*>(&n_cols), reinterpret_cast<MKL_INT**>(&rows_start),         \
...
#define ESL_IMPL_MKL_SPARSE_MM(fn, T)                                                              \
	::sparse_status_t mkl_sparse_mm(::sparse_operation_t operation, T alpha,                       \
		::sparse_matrix_t handle, const ::matrix_descr& descr, ::sparse_layout_t layout,           \
		const T* x, std::size_t columns, std::size_t l_dim_x, T beta, T* y, std::size_t l_dim_y)   \
	{                                                                                              \
		using TM = Mkl_type<T>;                                                                    \
		TM alpha_m, beta_m;                                                                        \
		...
		return ::fn(operation, alpha_m, handle, descr, layout, reinterpret_cast<const TM*>(x),     \
			static_cast<MKL_INT>(columns), static_cast<MKL_INT>(l_dim_x), beta_m,                  \
			reinterpret_cast<TM*>(y), static_cast<MKL_INT>(l_dim_y));                              \
	}
