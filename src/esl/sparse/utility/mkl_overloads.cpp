...
#define ESL_IMPL_MKL_SPARSE_CREATE_CSR(fn, T)                                                      \
	::sparse_status_t mkl_sparse_create_csr(::sparse_matrix_t& handle,                             \
											MKL_UINT n_rows,                                       \
											MKL_UINT n_cols,                                       \
											MKL_UINT* rows_start,                                  \
											MKL_UINT* rows_end,                                    \
	...
	{                                                                                              \
		return ::fn(&handle,                                                                       \
					...
					static_cast<MKL_INT>(n_rows),                                                  \
					static_cast<MKL_INT>(n_cols),                                                  \
					reinterpret_cast<MKL_INT*>(rows_start),                                        \
					reinterpret_cast<MKL_INT*>(rows_end),                                          \
					reinterpret_cast<MKL_INT*>(col_index),                                         \
...
#define ESL_IMPL_MKL_SPARSE_EXPORT_CSR(fn, T)                                                      \
	...
	{                                                                                              \
		return ::fn(handle,                                                                        \
					&indexing,                                                                     \
					reinterpret_cast<MKL_INT*>(&n_rows),                                           \
					reinterpret_cast<MKL_INT*>(&n_cols),                                           \
					reinterpret_cast<MKL_INT**>(&rows_start),                                      \
					reinterpret_cast<MKL_INT**>(&rows_end),                                        \
					reinterpret_cast<MKL_INT**>(&col_index),                                       \
					reinterpret_cast<Mkl_type<T>**>(&values));                                     \
...
#define ESL_IMPL_MKL_SPARSE_MM(fn, T)                                                              \
	::sparse_status_t mkl_sparse_mm(::sparse_operation_t operation,                                \
									T alpha,                                                       \
	...
	{                                                                                              \
		...
		return ::fn(operation,                                                                     \
					...
					descr,                                                                         \
					layout,                                                                        \
					reinterpret_cast<const TM*>(x),                                                \
					static_cast<MKL_INT>(columns),                                                 \
					static_cast<MKL_INT>(l_dim_x),                                                 \
					beta_m,                                                                        \
					reinterpret_cast<TM*>(y),                                                      \
