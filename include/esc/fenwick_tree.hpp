...
class Fenwick_tree
{
	static_assert(std::is_unsigned_v<Size_>);
...
public:
	...
	// Constructs the Fenwick tree of the given size holding zero values
	...
	Fenwick_tree(Input_iterator first, Input_iterator last) : data_(first, last)
	{
		assert(!is_empty());
		construct_in_place();
	}
...
private:
	...
	{
		const auto offset = CHAR_BIT * sizeof(Size) - esu::count_left_zeros(size() - 1);
