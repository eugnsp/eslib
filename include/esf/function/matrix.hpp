...
using Grad = esl::Matrix<std::conditional_t<Element::dim == 1, double, esl::Vector_2d>,
						 Quadr::size,
...
template<class Element,
		 class Quadr = esf::Quadr<2 * (Element::order - 1), Element::dim>,
		 class Func>
auto stiffness_matrix(const Grad<Element, Quadr>& grads,
					  Func func,
					  esl::Matrix_2d eps,
					  double scale)
{
