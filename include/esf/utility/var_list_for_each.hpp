...
{
	if constexpr (internal::is_dim1<Var_list>)
		internal::for_each_variable_and_element_impl2
			<Var_list, 0, Vertex_tag, Edge_tag>(fn);

	if constexpr (internal::is_dim2<Var_list>)
		internal::for_each_variable_and_element_impl2
			<Var_list, 0, Vertex_tag, Edge_tag, Face_tag>(fn);
}
