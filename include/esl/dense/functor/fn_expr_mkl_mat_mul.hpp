...
private:
	...
	void operator()(Expr_dst& 		dst,
					const Expr_src& src,
					Fn_expr_mkl_mat_mul_assign_tag) const
	{
	...
	void operator()(Expr_dst& 		dst,
					const Expr_src& src,
					Fn_expr_mkl_mat_mul_add_assign_tag) const
	{
					...
					const Beta 	 beta) const
	{
		...
		if constexpr (is_vector<Expr_dst> || is_vector<Expr2>)
			mkl_gemv<Layout_tag<Expr1>, No_transpose>(expr1.rows(),
													  expr1.cols(),
													  alpha,
													  expr1.data(),
													  expr1.lead_dim(),
			...
			mkl_gemm<Layout, Transp_a, Transp_b>(dst.rows(),
												 dst.cols(),
												 expr1.cols(),
												 alpha,
												 expr1.data(),
