...
public:
	Feast_interval_solver(
		const Sparse_matrix& mat_a,
		const Sparse_matrix& mat_b)
	:	mat_a_(mat_a),
		mat_b_(mat_b)
	{
		...
		bool  					user_init_guess = false)
	{
		...
		while (true)
		{
			...
			const auto status = mkl_feast_rci(job, n, ze, work1.data(), work2.data(),
				work3.data(), work4.data(), eps, n_loops, values_interval, m0,
				values.data(), vectors.data(), n_eigen_values_, rel_errors.data());
